...
def check_iter_unallowed_dict(path_file, struct, dates_format, report=None, log=None, date_parent=None, key=None, path_next=None):
...
def check_iter_unallowed_list(path_file, struct, dates_format, report=None, log=None, date_parent=None, key=None, path_next=None):
    validate_iter = False
            ...
            for date_regex in dates_format:
                if date_regex in struct_file:
                    ...
                    if date_parent is None:
                        ...
                        report.append([path_file, "Date file different", f"> {date_file} < different of parent date > {date_parent} <"])
                    else:
...
def check_unallowed(path, struct, dates_format, report=None, log=None, date_parent=None):
    ...
    for path_next in os.listdir(path):
        ...
        validate_iter = False
        for key in struct:
            if isinstance(struct[key], dict) and (key.startswith("^") and key.endswith("$")) and re.match(key, path_next) or path_next == key:
                report, log, validate_iter, date_parent = check_iter_unallowed_dict(path_file, struct, dates_format, report, log, date_parent, key, path_next)
            elif isinstance(struct[key], list):
                report, log, validate_iter, date_parent = check_iter_unallowed_list(path_file, struct, dates_format, report, log, date_parent, key, path_next)
            if validate_iter:
                break
        if validate_iter:
            continue
        if os.path.isfile(path_file):
            report.append([path_file, "Not allowed file", f"< {path_next} >. Allowed matches : {[key for key in struct.get('Files', []) + struct.get('Files_optionnal', [])]}"])
        elif os.path.isdir(path_file):
            report.append([path_file, "Not allowed folder", f"< {path_next} >. Allowed matches : {[key for key in struct.get('Files', []) + struct.get('Files_optionnal', [])] + [k for k in struct if k not in ['Files', 'Files_optionnal']]}"])
    return report, log
    ...
    if not len(dirs) and struct != {}:
        ...
        for key in struct:
            ...
            for path_next in dirs:
                ...
                if isinstance(struct[key], list):
                    for key2 in struct[key]:
                        for path_next2 in dirs:
                            ...
                            if key2.startswith("^") and key2.endswith("$") and re.match(key2, path_next2) or path_next2 == key2:
                                log["File presence validated"] = log.get("File presence validated", 0) + 1
                                break
                        else:
                            if key.endswith("_optionnal"):
                                continue
                            report.append([path, "Missing file matching", f"> {key2} <. Directory content : {dirs if len(dirs) else '> EMPTY <'}"])
                            breaker2 = True
                            break
                elif isinstance(struct[key], dict) and (key.startswith("^") and key.endswith("$")) and re.match(key, path_next) or path_next == key:
                    not_found = False
                ...
                if not_found and len(struct) > 0 and key not in ["Files", "Files_optionnal"] and key not in struct.get('Files_optionnal', []):
                    report.append([path, "Missing folder matching", f"> {key} <. Directory content : {dirs if len(dirs) else '> EMPTY <'}"])
    return report, log
